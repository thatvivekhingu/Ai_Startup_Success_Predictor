...
import { predictionAPI, analyticsAPI, gujaratAPI } from '../api';
...
const Dashboard = ({ onSelectPrediction }) => {
  ...
  // Gujarat Live News State
  const [gujaratNews, setGujaratNews] = useState([]);
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans pb-12">
      ...
      {/* ======================================================== */}
      {/* SECTION: LIVE GUJARAT STARTUP ECOSYSTEM PULSE & DEALS */}
      {/* ======================================================== */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
          <div className="space-y-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-indigo-50 border border-indigo-200 text-[10px] font-bold text-indigo-700 uppercase tracking-wider">
                Google News RSS & Notifier Engine
              </span>
            </div>
            <h2 className="text-2xl font-black text-slate-900 font-display tracking-tight">
              Gujarat Startup Ecosystem Pulse & Funding Deals
            </h2>
            <p className="text-xs text-slate-500 font-medium">
              Real-time syndicated updates across 12,900+ startups, 180+ incubators, and state innovation hubs (Ahmedabad, Gandhinagar, Surat, Rajkot).
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3 shrink-0">
            <button
              ...
              className="px-3.5 py-2 rounded-xl text-xs font-bold bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 flex items-center space-x-1.5 transition-colors"
            >
              <Activity className={`w-3.5 h-3.5 text-indigo-600 ${newsLoading ? 'animate-spin' : ''}`} />
              <span>{newsLoading ? 'Refreshing...' : 'Refresh Feed'}</span>
            </button>
          </div>
        </div>

        {/* State Ecosystem KPI Badges */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3.5 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Recognized Startups</span>
            <span className="text-xl font-black text-indigo-950 font-display mt-0.5">12,900+</span>
            <span className="text-[10px] text-indigo-600 font-semibold mt-0.5">DPIIT & Startup Gujarat</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-blue-50/50 border border-blue-100 flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Active Incubators</span>
            <span className="text-xl font-black text-blue-950 font-display mt-0.5">180+</span>
            <span className="text-[10px] text-blue-600 font-semibold mt-0.5">iCreate, GUSEC, PDEU, CIIE</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-emerald-50/50 border border-emerald-100 flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">STI Policy 2026–31</span>
            <span className="text-xl font-black text-emerald-950 font-display mt-0.5">₹1,000 Cr</span>
            <span className="text-[10px] text-emerald-600 font-semibold mt-0.5">DeepTech Innovation Fund</span>
          </div>

          <div className="p-3.5 rounded-2xl bg-amber-50/50 border border-amber-100 flex flex-col">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Student Prototypes</span>
            <span className="text-xl font-black text-amber-950 font-display mt-0.5">₹500 Cr</span>
            <span className="text-[10px] text-amber-600 font-semibold mt-0.5">SSIP 2.0 University Grants</span>
          </div>
        </div>

        {/* Live News Cards Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-1">
          {gujaratNews.map((news, idx) => (
            <a
              key={idx}
              href={news.link}
              target="_blank"
              rel="noopener noreferrer"
              className="p-4 rounded-2xl border border-slate-200 hover:border-indigo-400 hover:shadow-md transition-all bg-white flex flex-col justify-between group"
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 text-[10px] font-bold uppercase tracking-wider border border-indigo-100 truncate">
                    {news.category || 'Venture Deal'}
                  </span>
                ...
                <h3 className="text-xs font-bold text-slate-900 group-hover:text-indigo-600 transition-colors leading-snug line-clamp-3">
                  {news.title}
                </h3>
              </div>

              <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500 font-semibold mt-3">
                <span className="truncate">{news.source}</span>
                <span className="text-indigo-600 group-hover:translate-x-1 transition-transform flex items-center space-x-1 shrink-0">
